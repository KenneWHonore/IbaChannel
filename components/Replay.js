...
const Replay = () => {
    ...
    useEffect(() => {
        fetch(`https://www.googleapis.com/youtube/v3/search?part=id,snippet,contentDetails,status&channelId=${CHANNEL_ID}&maxResults=50&order=date&key=${API_KEY}`)
    ...
    useEffect(() => {
        if (videos && videos.length > 0) {
            ...
            fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics,contentDetails&id=${videoIds.join(',')}&maxResults=50&key=${API_KEY}`)
                .then(response => response.json())
                .then(data => {
                ...
                <FlatList data={videos2}
                          ...
                          renderItem={({item}) => (
                              <View style={styles.containImage}>
                                  <View style={{position: 'relative'}}>
                                      <TouchableOpacity>
                                          <Image source={{uri: item.snippet.thumbnails.high.url}}
                                                 style={styles.shortImage} filter="grayscale(0.5)"/>
                    ...
                    renderItem={({item}) => (
                        <View style={styles.NosFormationCard}>
                            ...
                            <Text style={{
                                ...
                                fontSize: 16,
                                marginTop: 5
                            }}>{decode(item.snippet.title).substring(0, 55) + '...'}</Text>
                        </View>
                    )}
                />
            </View>
